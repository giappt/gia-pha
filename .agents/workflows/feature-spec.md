...
2. **[SINH MA TRẬN TEST CASES BẰNG TEXT (GIVEN - WHEN - THEN)]:**
   - Tại **Mục 7 (Acceptance Criteria & Test Matrix)**: Bắt buộc định nghĩa danh sách các tiêu chí kiểm thử dưới dạng **Bộ 3 Đo Đếm Được** ở trạng thái chưa hoàn thành `- [ ]`:
     - **Given (Tiền điều kiện):** Trạng thái hệ thống / dữ liệu ban đầu.
     - **When (Hành động):** Thao tác chính xác của User hoặc API Trigger.
     - **Then (Kết quả kỳ vọng):** Sự thay đổi cụ thể trên DOM, Data DB/LocalStorage và Console log (0 lỗi).
   - Phân loại rõ: `Happy Path`, `Edge Case`, `Error Handling`.
